...
export function getApiBaseUrl(): string {
  ...
  if (typeof process !== 'undefined' && process.env) {
    return process.env.REACT_APP_API_BASE_URL || 'https://api.replyke.com/api/v7';
  }

  // Check Vite environment
  const viteEnv = getViteEnv();
  if (viteEnv) {
    return viteEnv.VITE_API_BASE_URL || 'https://api.replyke.com/api/v7';
  }

  // Fallback to default
  return 'https://api.replyke.com/api/v7';
}
